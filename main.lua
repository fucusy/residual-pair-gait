...
cmd:option('-modelname','residual','model name you want to load')
...
-- build the model
if opt.modelname == 'residual' then
    model, crit = model_residual(opt.gpu, opt.dropout)
    if opt.loadmodel ~= "" then
        model = torch.load(opt.loadmodel)
        info("load model from %s", opt.loadmodel)
    end
end
print(model)
